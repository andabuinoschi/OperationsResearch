...
class TwoPhase:
    ...
    def solve_two_phase_problem(self):
        self.solve_phase_1_problem(printing=True)
        tableau, basic_variables, m, n = self.solve_phase_2_problem(printing=True)
        return tableau, basic_variables, m, n
    ...
    def solve_phase_1_problem(self, printing=False):
        if printing:
            print("---------------------------------------------------------")
            print("------------------------ PHASE 1 ------------------------")
        ...
        if self.artificial_variables.size != 0:
        ...
        self.tableau, self.basic_variables = simplex.solve_simplex(printing=printing)
    ...
    def solve_phase_2_problem(self, printing=False):
        if printing:
            print("---------------------------------------------------------")
            print("------------------------ PHASE 2 ------------------------")
        # If the objective of the Simplex solution is not 0, then this means that the original problem is not
        # feasible.
        if self.tableau[self.m, self.n] != 0 and len(self.artificial_variables) != 0:
            return "The original problem is not feasible."
        else:
            if len(self.artificial_variables) != 0:
                ...
                if np.any(np.isin(self.basic_variables, self.artificial_variables)):
                    ...
                    index_in_all_original_variables = np.where(
                        np.logical_not(np.isin(self.all_variables, self.artificial_variables)))
                    # Get the index where we have artificial variables in the basis.
                    index_in_basic_artificial_variables = np.where(
                        np.isin(self.basic_variables, self.artificial_variables))
                    if np.any(
                            self.tableau[
                                index_in_basic_artificial_variables[0], index_in_all_original_variables[0]] != 0):
                        # Then we can replace the artificial variable with other non-basic original variable
                        indexes_valid_pivot = np.where(
                            self.tableau[
                                index_in_basic_artificial_variables[0], index_in_all_original_variables[0]] != 0)
                ...
                index_basic_variables_in_all = []
                for index in range(0, len(self.basic_variables)):
                    basic_variable = self.basic_variables[index]
                    all_variables = self.all_variables.tolist()
                    index_basic_var_in_all = all_variables.index(basic_variable)
                    index_basic_variables_in_all.append(index_basic_var_in_all)
                reduced_costs_basic_variables = self.tableau[self.m, index_basic_variables_in_all]
                ...
                if np.any(reduced_costs_basic_variables != 0):
                    ...
                    index_basic_variables_to_rewrite = np.array(index_basic_variables_in_all)[np.where(reduced_costs_basic_variables != 0)]
                    basic_variables_to_rewrite_in_all = self.all_variables[index_basic_variables_to_rewrite]
                    ...
                    for basic_variable in self.basic_variables:
                        if basic_variable in basic_variables_to_rewrite_in_all:
                            index_in_all_variables_to_rewrite.append(
                                np.min(np.where(self.all_variables == basic_variable)))
                    ...
                    self.tableau, self.basic_variables = simplex_solver.solve_simplex(additional_optimal_solution=False,
                                                                                      printing=printing)
        return self.tableau, self.basic_variables, self.m, self.n
...
if __name__ == '__main__':
    ...
    solution = two_phase.solve_two_phase_problem()
